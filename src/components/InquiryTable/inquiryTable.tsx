import { Table, Menu } from "@mantine/core"
//import { attachment } from "@pages/Dashboard/utils/inquiry.util"
import { SlOptionsVertical } from "react-icons/sl"
import { LuEye } from "react-icons/lu"
import { BiEditAlt } from "react-icons/bi"
import { Data } from "type/api/inquiry.types"
import dayjs from "dayjs"
import { formatInquiryType } from "@utils/helpers"

interface InquiryTableInterface {
    //setOpenRespond: React.Dispatch<React.SetStateAction<boolean>>
    data: Data[]
    handleInquiryDetails: (val: Data) => void
    showTalent?: boolean
    isClient?: boolean
}
const InquiryTable = ({
    //setOpenRespond,
    data,
    handleInquiryDetails,
    showTalent = true,
    isClient
}: InquiryTableInterface) => {
    const rows = data.map((item, index) => (
        <Table.Tr
            key={index}
            className="text-[#1C1C1C]  group "
            style={{
                fontSize: "14px",
            }}
        >
            <Table.Td>{item.fullName}</Table.Td>
            <Table.Td>{item.emailAddress}</Table.Td>
            {showTalent && (
                <Table.Td>
                    {item.bookedForTalent.firstName +
                        " " +
                        item.bookedForTalent.lastName}
                </Table.Td>
            )}
            <Table.Td>{item.inquiryType}</Table.Td>
            <Table.Td>{item.subject}</Table.Td>
            {/* <Table.Td>{attachment(item.attachment)}</Table.Td> */}
            {/* <Table.Td>
                {item.eventDate.map((element, index) => (
                    <span>
                        {dayjs(element.date).format("D MMM, YYYY ")}
                        {index < item.eventDate.length - 1 && ", "}
                    </span>
                ))}
            </Table.Td> */}
            <Table.Td>{dayjs(item.createdAt).format("D MMM, YYYY ")}</Table.Td>
            <Table.Td className="cursor-pointer">
                <Menu shadow="md" width={250} position="bottom-end">
                    <Menu.Target>
                        <div>
                            <SlOptionsVertical />
                        </div>
                    </Menu.Target>
                    <Menu.Dropdown className="rounded-lg ">
                        <Menu.Item
                            className="font-medium text-md rounded-lg"
                            onClick={() => handleInquiryDetails(item)}
                        >
                            <div className="flex items-center">
                                <LuEye size="22px" color="#373636" />
                                <p className="text-[#373636] ml-2">
                                    View full details{" "}
                                </p>
                            </div>
                        </Menu.Item>

                        {!isClient && <Menu.Item
                            className="font-medium text-md rounded-lg"
                            onClick={() => handleInquiryDetails(item)}
                        >
                            <div className="flex items-center">
                                <BiEditAlt size="22px" color="#373636" />
                                <p className="text-[#373636] ml-2">Respond</p>
                            </div>
                        </Menu.Item>}
                    </Menu.Dropdown>
                </Menu>
            </Table.Td>
        </Table.Tr>
    ))
    const tableHead = [
        "Name",
        "Email",
        "Talent",
        "Type",
        "Subject",
        // "Attachment",
        "Date",
        "Action",
    ]
    return (
        <div>
            <div className="hidden md:block">
                <Table
                    style={{
                        fontFamily: "Montserrat",
                        fontSize: "14",
                        fontWeight: "semi-",
                    }}
                    verticalSpacing="md"
                    data-testid="table-data"
                    role="grid"
                    withRowBorders={true}
                >
                    <Table.Thead className=" w-full border-b-[1px] border-[#EEEEEE] font-bold">
                        <Table.Tr className="">
                            {tableHead.map((item, index) => {
                                if (!(!showTalent && item === "Talent"))
                                    return (
                                        <Table.Th
                                            key={index}
                                            style={{
                                                borderBottom: "1px",
                                                color: "#000000",
                                                fontSize: "14px",
                                                fontWeight: 600,
                                            }}
                                        >
                                            {item}
                                        </Table.Th>
                                    )
                            })}
                        </Table.Tr>
                    </Table.Thead>
                    <Table.Tbody>{rows}</Table.Tbody>
                </Table>
            </div>
            <div className="block md:hidden">
                {data.map((item, index) => (
                    <div
                        key={index}
                        className="border bg-white-100
                rounded-md p-4 mb-2"
                    >
                        <div className="flex justify-between">
                            <div className="flex justify-between">
                                <p className="font-medium text-2md mr-2">
                                    {item.fullName}
                                </p>
                                {formatInquiryType(item.inquiryType)}
                            </div>

                            <div className="cursor-pointer">
                                <Menu
                                    shadow="md"
                                    width={250}
                                    position="bottom-end"
                                >
                                    <Menu.Target>
                                        <div>
                                            <SlOptionsVertical />
                                        </div>
                                    </Menu.Target>
                                    <Menu.Dropdown className="rounded-lg ">
                                        <Menu.Item
                                            className="font-medium text-md rounded-lg"
                                            onClick={() =>
                                                handleInquiryDetails(item)
                                            }
                                        >
                                            <div className="flex items-center">
                                                <LuEye
                                                    size="22px"
                                                    color="#373636"
                                                />
                                                <p className="text-[#373636] ml-2">
                                                    View full details{" "}
                                                </p>
                                            </div>
                                        </Menu.Item>

                                        {!isClient &&  <Menu.Item
                                            className="font-medium text-md rounded-lg"
                                            onClick={() =>
                                                handleInquiryDetails(item)
                                            }
                                        >
                                            <div className="flex items-center">
                                                <BiEditAlt
                                                    size="22px"
                                                    color="#373636"
                                                />
                                                <p className="text-[#373636] ml-2">
                                                    Respond
                                                </p>
                                            </div>
                                        </Menu.Item>}
                                    </Menu.Dropdown>
                                </Menu>
                            </div>
                        </div>
                        
                        <div className="mt-3">
                            <p className="font-medium text-md">
                                {item.emailAddress}
                            </p>
                        </div>
                        <p className="font-normal text-[12px] leading-3 mt-2">
                            {item.subject}
                        </p>
                        <div className="mt-3">
                            <p className="font-medium text-md">
                                {dayjs(item.createdAt).format("DD MMM, YYYY")}
                            </p>
                        </div>
                    </div>
                ))}
            </div>
        </div>
    )
}

export default InquiryTable
