import {
    ...
    CreateTalentRequest,
    CreateTalentResponse,
    IndustriesResponse,
    TalentsResponse,
    TalentStatsResponse,
} from "type/api/talents.types"
...
export const fetchTalents = async (filters?: {
    ...
    const response = await axiosInstance.get<TalentsResponse>(
        `${ENDPOINT}/talents`,
...
export const fetchTalentStats = async () => {
    const response = await axiosInstance.get<TalentStatsResponse>(
        ENDPOINT + `/talent-stats`
    )
    return response.data
}
export const createTalent = (data: CreateTalentRequest) => {
    return axiosInstance.post<CreateTalentResponse>(ENDPOINT + "/talents", data)
}
...
export const editTalent = async ({ id, ...data }: CreateTalentRequest) => {
    return axiosInstance.patch<CreateTalentResponse>(`/talents/${id}`, data)
}
export const deleteTalent = async (id: string) => {
    return axiosInstance.delete<CreateTalentResponse>(
        `${ENDPOINT}/talents/${id}`
    )
}
